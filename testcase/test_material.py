...
class TestMaterial:
    ...
    e1 = expect.get_expect("material", "is_material_exists", "e1")
    e2 = expect.get_expect("material", "is_material_exists", "e2")
    e3 = expect.get_expect("material", "is_material_exists", "e3")
    e4 = expect.get_expect("material", "is_material_exists", "e4")
    e5 = expect.get_expect("material", "is_material_exists", "e5")
    ...
    def test_is_material_exits_normal(self, pre_condition):
        res = self.material.is_material_exists(name=self.exits_l1[0], code=self.exits_l1[1], versions=self.exits_l1[2])
        assert_that(res, equal_to(self.e1))

    @pytest.mark.parametrize(
        "v_list,expect", [[exits_l2, e2], [exits_l3, e3], [exits_l4, e4], [exits_l5, e5]],
